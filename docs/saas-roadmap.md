...
|---|---|---|---|---|
| 1 | **Subdomain-Routing** (Wildcard-DNS + TLS via Cloudflare; Tenant aus Host statt Header) | M | mittel | ✅ Code: Tenant aus Subdomain (`team.localhost:3000` lokal), Switcher/„Open" navigieren zur Subdomain + Cross-Origin-Login-Handoff, CORS für `*.localhost`/`*.synqed.org`. **Offen für Prod:** Wildcard-DNS + TLS in Cloudflare/Railway |
...
| 3 | **Onboarding / Control-Plane** (`synqed.org`: Sign-up, „Add to Discord", Team+Subdomain anlegen) | L | mittel | ✅ `/control`: Login + Team anlegen + Teams-Liste + **„Connect Discord"** (Invite mit signiertem State → Callback bindet guild_id). **Offen für Prod:** echte DISCORD_CLIENT_ID/SECRET |
| 4 | **Bot multi-guild** + per-Org-Config + Scheduler-Tick | L | mittel | ✅ Code: `config` context-aware (per-Org, löst Singleton), Bot löst Org aus `guildId` auf, actions per-Org + Membership, Scheduler als per-Org-Minuten-Tick (Timing unit-getestet). **Offen:** Live-Discord-Verifikation (Bot muss laufen) + ggf. Channel-Auflösung für Main/Academy auf *einer* Guild |
| 5 | **`settings` pro Org** | M | mittel | ✅ `Setting` tenant-gescopet; Settings-API liest/schreibt pro Org (`getSettingsForCurrentOrg`/`saveSettingsForCurrentOrg`). **Offen:** der globale `config`-Singleton/Scheduler spiegelt weiter nur die Default-Org — wird mit #4 (per-Org-Scheduler) aufgelöst |
| 6 | **optionalAuth-Reads absichern** + Dev-Features gaten | S–M | niedrig | ✅ scrims + schedule-details brauchen jetzt Auth + Membership (kein Cross-Tenant-Read). Switcher/Dev-Login/`?tenant`-Override nur in `import.meta.env.DEV`. **Offen:** `settings`/`user-mappings` GET bleiben public (Login braucht sie); Cross-Subdomain-Login-Handoff prod-korrekt (server-Einmal-Code / httpOnly-Cookie auf `.synqed.org`); actions/admin-Routes-Membership-Check |
